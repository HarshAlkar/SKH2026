...
class DoctorService {
  ...
  Future<Map<String, dynamic>> acceptConsultation(String consultationId) async {
    final response = await _apiService.post('/consultations/$consultationId/accept/');
  ...
  Future<Map<String, dynamic>> smartMatch(String symptoms) async {
    // MOCKED BACKEND RESPONSE UNTIL API IS READY
    // Simulates POST /api/consultations/smart-match/
    ...
    int doctorId = 0; // We'll try to find an existing doctor to map to
    String doctorName = 'Dr. Amit Sharma';
    ...
    // Try to find a real doctor from the backend to make it work realistically
    ...
    if (urgency == 'EMERGENCY') {
      recommendedDate = now; // Today
      recommendedTime = '${(now.hour + 1).toString().padLeft(2, '0')}:00:00'; // Next hour
    }
