...
class DoctorAppointmentService {
  ...
  Future<DoctorAppointment?> acceptAppointment(int appointmentId) async {
    try {
      final response = await _api.post('/consultations/appointments/$appointmentId/accept/');
      if (response is Map) {
        return DoctorAppointment.fromJson(Map<String, dynamic>.from(response));
      }
      return null;
    } catch (e) {
      // Queue for sync when offline / network error.
      debugPrint('Accept online failed, queueing: $e');
      await OfflineApi.instance.post('/consultations/appointments/$appointmentId/accept/', body: {});
      return DoctorAppointment(
        id: appointmentId,
        patientId: 0,
        patientName: 'Queued offline',
        age: '—',
        gender: 'Not set',
        village: '—',
        phoneNumber: '',
        ...
        type: DoctorConsultationType.video,
        status: 'ACCEPTED',
        historySummary: 'Accept queued offline — will sync when online.',
        notes: '',
      );
    }
  }
  ...
  Future<bool> rejectAppointment(int appointmentId, {String? reason}) async {
    try {
      await _api.post(
        '/consultations/appointments/$appointmentId/reject/',
